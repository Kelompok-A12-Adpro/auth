// src/service/user_service.rs - Fixed JWT creation
    ...
    match find_user_by_email(trimmed_email).await {
        Some(user) => {
            if verify_password(&req.password, &user.password) {
                // 🔥 PERBAIKAN: Kirim seluruh object user, bukan hanya email
...
// 🔥 PERBAIKAN UTAMA: JWT sekarang menyertakan semua data yang dibutuhkan
fn create_jwt_token(user: &crate::model::user::User) -> Result<String, String> {
